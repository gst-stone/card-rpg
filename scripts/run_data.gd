...
func add_gold(amount: int) -> void: gold += max(0, amount)
func spend_gold(amount: int) -> bool:
	if amount < 0 or gold < amount: return false
	gold -= amount; return true
func add_card(card_name: String) -> void: deck.append(card_name)
func remove_card(card_name: String) -> bool:
	var index := deck.find(card_name)
	if index < 0: return false
	deck.remove_at(index); return true
func heal(amount: int) -> void: player_hp = min(max_hp, player_hp + max(0, amount))
func take_damage(amount: int) -> void: player_hp = max(0, player_hp - max(0, amount))
...
func apply_relics() -> void:
	...
	if max_hp > old_max: player_hp += max_hp - old_max
